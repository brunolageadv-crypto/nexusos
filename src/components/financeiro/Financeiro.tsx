...
/* ═══ FormTransacao ══════════════════════════════════════════ */
function FormTransacao({cats,initial,onSave,onClose}:{cats:Categoria[];initial?:Partial<Transacao>;onSave:(t:Transacao)=>void;onClose:()=>void}) {
  const [form,setForm]=useState({
    data:initial?.data??todayISO(),descricao:initial?.descricao??'',valor:initial?.valor??0,
    tipo:(initial?.tipo??'despesa') as Tipo,categoria:initial?.categoria??'',subcategoria:initial?.subcategoria??'',
    positivo:initial?.positivo??false,observacao:initial?.observacao??'',
  })
      ...
      <div style={{background:'var(--bg-2)',border:'1px solid var(--border-md)',borderRadius:16,width:'100%',maxWidth:520,maxHeight:'90vh',overflowY:'auto',boxShadow:'0 32px 80px rgba(0,0,0,0.6)'}}>
        ...
        <div style={{padding:'18px 22px'}}>
          ...
          <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:10}}>
            ...
            <FL label="Valor (R$)"><input type="number" step="0.01" min={0} style={inp} value={form.valor||''} onChange={f('valor')} placeholder="0,00"/></FL>
          </div>
          <FL label="Descrição"><input style={inp} value={form.descricao} onChange={f('descricao')} placeholder="Ex: Conta de luz, Salário…"/></FL>
          <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:10}}>
            <FL label="Categoria"><select style={sel} value={form.categoria} onChange={f('categoria')}>
              <option value="">Selecione…</option>
              {cats.map(c=><option key={c.nome}>{c.nome}</option>)}
            </select></FL>
            <FL label="Subcategoria"><select style={sel} value={form.subcategoria} onChange={f('subcategoria')}>
              <option value="">Selecione…</option>
          ...
          <div style={{display:'flex',gap:8,justifyContent:'flex-end',marginTop:8}}>
            ...
            <button onClick={()=>onSave({id:(initial as any)?.id??newId(),criadoEm:(initial as any)?.criadoEm??new Date().toISOString(),...form,valor:Number(form.valor)})} style={{padding:'9px 18px',borderRadius:8,border:'1px solid rgba(0,229,255,0.4)',background:'rgba(0,229,255,0.1)',color:'var(--text-accent)',fontFamily:'var(--font-display)',fontWeight:700,cursor:'pointer'}}>Salvar</button>
          </div>
        </div>
      </div>
    </div>
...
function FormContaPagar({cats,initial,onSave,onClose}:{cats:Categoria[];initial?:Partial<ContaPagar>;onSave:(c:ContaPagar)=>void;onClose:()=>void}) {
  const [form,setForm]=useState({descricao:initial?.descricao??'',valor:initial?.valor??0,vencimento:initial?.vencimento??'',categoria:initial?.categoria??'',recorrente:initial?.recorrente??false,pago:initial?.pago??false})
    ...
    <div style={{position:'fixed',inset:0,background:'rgba(0,0,0,0.7)',zIndex:200,display:'flex',alignItems:'center',justifyContent:'center',padding:16}}>
      <div style={{background:'var(--bg-2)',border:'1px solid var(--border-md)',borderRadius:16,width:'100%',maxWidth:460,boxShadow:'0 32px 80px rgba(0,0,0,0.6)'}}>
        ...
        <div style={{padding:'18px 22px'}}>
          ...
          <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:10}}>
            <FL label="Valor"><input type="number" step="0.01" style={inp} value={form.valor||''} onChange={f('valor')}/></FL>
          ...
          <div style={{display:'flex',gap:8,justifyContent:'flex-end',marginTop:16}}>
            <button onClick={onClose} style={{padding:'9px 18px',borderRadius:8,border:'1px solid var(--border)',background:'none',color:'var(--text-secondary)',fontFamily:'var(--font-display)',fontWeight:600,cursor:'pointer'}}>Cancelar</button>
            <button onClick={()=>onSave({id:(initial as any)?.id??newId(),criadoEm:(initial as any)?.criadoEm??new Date().toISOString(),...form,valor:Number(form.valor)})} style={{padding:'9px 18px',borderRadius:8,border:'1px solid rgba(245,158,11,0.4)',background:'rgba(245,158,11,0.1)',color:'#f59e0b',fontFamily:'var(--font-display)',fontWeight:700,cursor:'pointer'}}>Salvar</button>
          </div>
        </div>
      </div>
    </div>
...
/* ═══ Main ═══════════════════════════════════════════════════ */
type FinTab='visao'|'transacoes'|'contas'|'relatorios'
...
export default function Financeiro() {
  ...
  return(
    <div style={{display:'flex',flexDirection:'column',height:'100%',background:'var(--bg-0)'}}>
      ...
      {modalT&&<FormTransacao cats={cats} initial={editT??undefined} onSave={async t=>{await saveT(t);setModalT(false);setEditT(null)}} onClose={()=>{setModalT(false);setEditT(null)}}/>}
