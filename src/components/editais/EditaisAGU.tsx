...
const STATUS_CFG: { value: StatusMaterial; label: string; color: string }[] = [
...
function StatusBadge({ status, onClick }: { status: StatusMaterial; onClick?: () => void }) {
  const s = STATUS_CFG.find(x => x.value === status)!
  return (
...
interface SimField { key: keyof SimuladoEntry; label: string; type: string }

const SIM_FIELDS: SimField[] = [
  { key: 'data',          label: 'Data',          type: 'date'   },
  { key: 'banca',         label: 'Banca',         type: 'text'   },
  { key: 'totalQuestoes', label: 'Total Questões', type: 'number' },
  { key: 'acertos',       label: 'Acertos',        type: 'number' },
  { key: 'erros',         label: 'Erros',          type: 'number' },
  { key: 'branco',        label: 'Em Branco',      type: 'number' },
  { key: 'tempoMinutos',  label: 'Tempo (min)',     type: 'number' },
]
...
function SimuladosModal({ simulados, onSave, onDelete, onClose }: {
  ...
  const emptyForm: SimuladoEntry = { id: '', data: '', banca: '', totalQuestoes: 0, acertos: 0, erros: 0, branco: 0, tempoMinutos: 0, pioresDisciplinas: '', observacoes: '' }
  const [form, setForm] = useState<SimuladoEntry>(emptyForm)
  const [editing, setEditing] = useState(false)

  const pct = form.totalQuestoes > 0 ? Math.round((form.acertos / form.totalQuestoes) * 100) : 0

  function setField(key: keyof SimuladoEntry, val: string | number) {
    setForm(f => ({ ...f, [key]: val }))
  }
        ...
        {editing && (
          <div style={{ background: 'var(--surface2)', borderRadius: 10, padding: 16, marginBottom: 16, display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10 }}>
            ...
            <label style={{ gridColumn: '1/-1', display: 'flex', flexDirection: 'column', gap: 4 }}>
              ...
              <input value={form.pioresDisciplinas} onChange={e => setField('pioresDisciplinas', e.target.value)}
                style={{ padding: '6px 10px', borderRadius: 6, border: '1px solid var(--border)', background: 'var(--surface)', color: 'var(--text-primary)', fontSize: 13 }} />
            </label>
            <label style={{ gridColumn: '1/-1', display: 'flex', flexDirection: 'column', gap: 4 }}>
            ...
            {pct > 0 && (
              <div style={{ gridColumn: '1/-1', fontSize: 12, color: 'var(--text-muted)' }}>
                % Acerto: <strong style={{ color: pct >= 70 ? 'var(--green)' : 'var(--red)' }}>{pct}%</strong>
              </div>
            )}
            <div style={{ gridColumn: '1/-1', display: 'flex', gap: 8 }}>
              ...
              <button onClick={() => { setForm(emptyForm); setEditing(false) }} style={{ padding: '7px 18px', borderRadius: 8, background: 'var(--surface)', color: 'var(--text-muted)', border: '1px solid var(--border)', cursor: 'pointer', fontSize: 13 }}>Cancelar</button>
            </div>
          </div>
        )}
          ...
          : simulados.map(s => {
              ...
              return (
                <div key={s.id} style={{ border: '0.5px solid var(--border)', borderRadius: 10, padding: '12px 14px', marginBottom: 8, display: 'flex', gap: 12, alignItems: 'center' }}>
                  ...
                  <button onClick={() => { setForm(s); setEditing(true) }} style={{ background: 'none', border: 'none', cursor: 'pointer', fontSize: 14 }}>✏️</button>
...
export default function EditaisAGU() {
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 0 }}>
      ...
      <div style={{ background: 'var(--surface)', border: '0.5px solid var(--border)', borderRadius: 12, padding: '16px 20px', marginBottom: 16 }}>
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexWrap: 'wrap', gap: 12 }}>
          <div>
            ...
            <p style={{ fontSize: 12, color: 'var(--text-muted)', marginTop: 2 }}>14 disciplinas · {TOTAL_SUBTOPICOS} subtópicos</p>
          </div>
        ...
        <div style={{ marginTop: 14 }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 6 }}>
            <span style={{ fontSize: 11, color: 'var(--text-muted)' }}>Progresso geral</span>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(120px, 1fr))', gap: 8, marginTop: 12 }}>
          {([
            ['Concluídos',    String(stats.concluidos),    'var(--green)'],
            ['Em andamento',  String(stats.iniciados),     'var(--amber)'],
            ['Questões',      String(stats.totalQuestoes), 'var(--purple)'],
            ...
            <div key={l} style={{ background: 'var(--surface2)', borderRadius: 8, padding: '8px 12px' }}>
              ...
              <p style={{ fontSize: 18, fontWeight: 700, fontFamily: "'Space Mono',monospace", color: c }}>{v}</p>
            </div>
          ))}
        </div>
      </div>
              ...
              return (
                <button key={d.id} onClick={() => setActiveDisciplina(d.id)}
                  style={{ width: '100%', display: 'flex', alignItems: 'center', gap: 8, padding: '7px 8px', borderRadius: 7, border: 'none',
                    ...
                    cursor: 'pointer', fontSize: 12, fontWeight: activeDisciplina === d.id ? 700 : 400, textAlign: 'left', marginBottom: 1, transition: 'all 0.15s' }}>
                  <span style={{ width: 8, height: 8, borderRadius: '50%', background: d.cor, flexShrink: 0 }} />
                  <span style={{ flex: 1, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{d.nome.replace('Direito ', '')}</span>
              ...
              return (
                <div key={topico.id} style={{ background: 'var(--surface)', border: '0.5px solid var(--border)', borderRadius: 10, marginBottom: 8, overflow: 'hidden' }}>
                  <button onClick={() => toggleTopico(topico.id)}
                    ...
                    <span style={{ color: 'var(--text-muted)', fontSize: 11, transition: 'transform 0.2s', transform: expanded ? 'rotate(90deg)' : 'rotate(0deg)', display: 'inline-block' }}>▶</span>
                  ...
                  {expanded && (
                    <div style={{ borderTop: '0.5px solid var(--border)' }}>
                      <div style={{ display: 'grid', gridTemplateColumns: '1fr 110px 110px 60px 80px 80px 60px', gap: 8, padding: '6px 14px', background: 'var(--surface2)', fontSize: 10, fontWeight: 700, color: 'var(--text-muted)', textTransform: 'uppercase', letterSpacing: 0.8 }}>
                        <span>Subtópico</span><span>Material</span><span>Resumo</span><span>Fichado</span><span>Questões</span><span>Acertos</span><span>% Acerto</span>
                      </div>
      ...
      {showSimulados && (
        <SimuladosModal simulados={simulados} onSave={saveSimulado} onDelete={deleteSimulado} onClose={() => setShowSimulados(false)} />
      )}
    </div>
  )
}
